...
\author{Kidus Asfaw\\Univ. of Michigan \And
        Joonha Park\\Univ. of Kansas \And
        Aaron A. King\\Univ. of Michigan \And
        Edward L. Ionides\\Univ. of Michigan}
\Plainauthor{Kidus Asfaw, Joonha Park, Aaron A. King and Edward L. Ionides}

\title{Partially observed Markov processes with spatial structure via the \proglang{R} package \pkg{spatPomp}\\}
\Plaintitle{Spatially Coupled Partially Observed Markov Processes via the R Package spatPomp}
\Shorttitle{\pkg{{\spatPomp}}: Spatially Coupled Partially Observed Markov Processes in \proglang{R}}


\Abstract{
We introduce a computational framework for modeling and statistical inference on high-dimensional dynamic systems.
...
\noindent
This version was compiled on {\today} using \pkg{spatPomp} \Sexpr{packageVersion("spatPomp")} with \pkg{pomp} \Sexpr{packageVersion("pomp")} and \proglang{R} \Sexpr{paste0(R.version$major,".",R.version$minor)} from source code at \code{https://github.com/ionides/spatPomp-article}.
It is provided under the Creative Commons Attribution License. 
}

\Keywords{Markov processes, hidden Markov model, state space model, stochastic dynamical system, maximum likelihood, plug-and-play, spatiotemporal data, mechanistic model, sequential Monte Carlo, \proglang{R}}
\Plainkeywords{Markov processes, hidden Markov model, state space model, stochastic dynamical system, maximum likelihood, plug-and-play, spatiotemporal data, mechanistic model, sequential Monte Carlo, R}

\Address{
  Kidus Asfaw, Edward L. Ionides\\
  University of Michigan\\
  48109 Michigan, United States of America\\
  E-mail: \email{kasfaw@umich.edu}, \email{ionides@umich.edu}\\
  ...
  E-mail: \email{j.park@ku.edu}\\
  URL: \url{https://people.ku.edu/~j139p002}\\
...
This paper describes the \pkg{spatPomp} package \citep{R:spatPomp}, available for \proglang{R} \citep{R} from the Comprehensive R Archive Network (CRAN).
The objective of the \pkg{\spatPomp} package is to facilitate model development and data analysis in the context of the general class of SpatPOMP models, enabling scientists to separate the scientific task of model development from the statistical task of providing inference tools.
...
The \pkg{spatPomp} package has been used to develop and demonstrate plug-and-play methodology for SpatPOMP models \citep{ionides21,ionides22} and scientific applications of these methods are anticipated.
...
Finally, \cref{sec:conclusion} discusses extensions and applications of \pkg{\spatPomp}.
...
The adapted bagged filter \citep{ionides21} combines many independent particle filters.
...
ABF can be combined with the guided intermediate resampling technique used by GIRF to give an algorithm called ABF-IR \citep{ionides21} implemented as \code{abfir}.
...
ABF satisfies such constraints but has been found to have higher variance than BPF on some benchmark problems \citep{ionides21}.
For the measles model built by \code{measles()}, BPF and ABF have been found to perform better than EnKF and GIRF \citep{ionides21}.
For the Lorenz-96 example built by \code{lorenz()}, GIRF and BPF perform well \citep{ionides21}.
...
Here, we present iterated versions of GIRF, EnKF and the unadapted bagged filter (UBF), a version of ABF with $\Np=1$. These algorithms are known as {\iGIRF} \citep{park20}, {\iEnKF} \citep{li20} and {\iUBF} \citep{ionides21} respectively.
We also discuss a recently proposed iterated block particle filter, IBPF \citep{ning21-ibpf,ionides22}.
...
The matrix $\sigma_{0:\Time,1:\Thetadim}$ can be constructed using the \code{rw_sd} function, which simplifies the construction of the \code{rw.sd} argument for regular parameters and IVPs.
...
The success of \code{bpfilter} on a variety of spatiotemporal models \citep{ionides21} raises the question of how to extend a block particle filter for parameter estimation.
An iterated filtering algorithm accommodating the structure of the block particle filter was proposed by \citet{ionides22}.
A previous proposal by \citet{ning21-ibpf} addresses the special case where all parameters are unit-specific.
...
A tutorial guide to \code{ibpf} is available on the \pkg{spatPomp} website \citep{ning22-tutorial}.
...
UBF and IUBF were found to be effective on the measles model \citep{ionides21} and \code{iubf} was developed with this application in mind.
...
For \code{measles()}, the data consist of biweekly counts and parameters are all shared between units, matching the analysis of \citet{park20} and \citet{ionides21}.
...
<<measles_sim_plot,fig.width=7, fig.height=3.5,out.width = '90%', fig.align='center',fig.show='hold',echo=F,eval=T,cache=F,fig.cap=r"{A: reported measles cases in two week intervals for the six largest cities in England, \code{plot(measles6,log=TRUE)}. B: simulated data, \code{plot(simulate(measles6),log=TRUE)}. The vertical scale is \code{log(cases+1)}.}">>=
...
To date, \pkg{\spatPomp} has focused on algorithms with the plug-and-play property, but the package is designed to support new algorithms with and without this property.
...
Since the mission of \pkg{spatPomp} is to be a home for such analyses, the package developers welcome contributions and collaborations to further expand the functionality of \pkg{\spatPomp}.
...
Recent studies have reiterated the need for such methods, and made some progress toward attaining them \citep{becker16,li20}.
The \pkg{\spatPomp} package offers new opportunities for the development and application of models, statistical methodology and data analysis workflows for model-based investigations of spatiotemporal systems.
...
We appreciate constructive feedback received from anonymous editors and reviewers.
