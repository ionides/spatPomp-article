...
\newcommand\class[1]{class \code{\textquotesingle #1\textquotesingle}}
\newcommand\Class[1]{Class \code{\textquotesingle #1\textquotesingle}}
...
The RPs are provided to \code{rinit} so they can also participate in the initialization.

For a stationary model, the initial state may be set at the stationary distribution by specifying $t_0$ sufficiently remote from $t_1$ to allow the system to equilibrate.
\citet{stocks20} provides an example of this using \pkg{pomp}.
When following this approach, the model is insensitive to the choice of $f_{\vec{X}_0}(\vec{x}_0\giventh\vectheta)$.
...
A related approach is the synthetic likelihood method of \cite{wood10} which can be implemented using \code{probe.match()}.
However, ABC and synthetic likelihood are feature-based methods that may lose substantial information compared to full-information methods that work with the likelihood function rather than a summary statistic \citep{fasiolo16}.
...
The convergence record of parameter estimates and their likelihood evaluations is stored in the \code{traces} slot of the resulting \class{igirfd\_spatPomp} object. 
The \code{plot} method for this class gives a graphical representation of the convergence traces, as shown in \cref{fig:bm_igirf_convergence2}.
We see that this \code{igirf} search has allowed us to explore the parameter space and climb significantly up the likelihood surface to within a small neighborhood of the maximum likelihood.
...
Each row of \code{rho\_prof} now contains a parameter estimate and its log-likelihood, with $\rho$ values fixed along a grid.
...
\]
Here, $N_{SE,\unit}(t)$, $N_{EI,\unit}(t)$, and $N_{IR,\unit}(t)$ are counting processes corresponding to the cumulative number of individuals transitioning between the compartments identified by the subscripts.
...
We construct the model described in \cref{sec:measles-math-model} for the simplified situation where $\alpha_{\unit}=1$, $\measlesImmigration_{\unit}=0$ and $\cohort_{\unit}=0$.
...
The arguments \code{unit\_ivpnames = c(\textquotesingle S\textquotesingle,\textquotesingle E\textquotesingle, \textquotesingle I\textquotesingle)} and \code{unit\_statenames = c(\textquotesingle S\textquotesingle,\textquotesingle E\textquotesingle,\textquotesingle I\textquotesingle,\textquotesingle C\textquotesingle)} enable \code{spatPomp()} to expect these variables and define then as needed when compiling the Csnippets.
Similarly, \code{unit\_covarnames = \textquotesingle P\textquotesingle} declares the corresponding unit-specific population covariate.
...
It is convenient (but not necessary) to supply a parameter vector for testing the model.
...
This tutorial was supported by National Science Foundation grants DMS-1761603 and DMS-1646108, and National Institutes of Health grants 1-U54-GM111274 and 1-U01-GM110712.
We recognize those who have participated in the development and testing of \pkg{spatPomp}, especially Allister Ho, Zhuoxun Jiang, Jifan Li, Patricia Ning, Eduardo Ochoa, Rahul Subramanian and Jesse Wheeler. Ben Bolker provided feedback incorporated into this version of the article.
